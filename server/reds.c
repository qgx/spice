...
int get_content()
{
	...
	int ret = 0;
	 char buffer[21];
	if((fd=open("/config/etc/mcvda_mutil_clients.conf",O_RDONLY)) == -1)
	{
	    spice_warning( "open /config/etc/mcvda_mutil_clients.conf error\n\a");
	    return 0;
	}

	if((size=read(fd,buffer,21)) < 21)
	{
	    spice_warning( "read file error!\n");
	    goto CONTENT_ERROR;
	}

	if(memcmp(buffer,"MCVDA_MUTIL_CLIENTS=",20) != 0)
	{
	    spice_warning( "file content error!\n");
          goto CONTENT_ERROR;
	}

	int imc;
	if(sscanf(buffer+20,"%1d",&imc) != 1)
	{
	    spice_warning( "file content error1!\n");
	    goto CONTENT_ERROR;
	}
...
//reds->allow_multiple_clients = getenv(SPICE_DEBUG_ALLOW_MC_ENV) != NULL;
    reds->allow_multiple_clients = get_content();
    if (reds->allow_multiple_clients) {
